...
##################### split dataset ############################
################################################################
spl.var <- "C_Water"							## by which variables should the dataset be split? leave empty c() or set to NULL for *no* splitting by class variables, or provide one or more valid class variables to split along these variables; e.g. "C_Group", or c("C_Group", "C_Mode", ...)
...
#######
dpt.pre <- "snv"							## Character vector, which of the available modules of data pre-treatments to apply. Leave at NULL for no additional data pre-treatment. For possible values see ?dpt_modules.
#######
...
# plotting 
pca.colorBy <- c("C_Water", "C_waterTemp", "C_conSNr")	## which variables to use for coloring the score plots. leave empty c() or set to NULL for using all available class-variables for coloring. Provide a character vector with valid class variable names to color along these variables; e.g. c("C_Group", "C_Temp", "C_RelHum", ...)
...
pca.sc.pairs <- 1:5 					## Numeric sequence of length >=2, indicating what PC components to plot in the score pairs plot. Set to NULL for not plotting the pairs plot.
pca.lo <- 1:5							## Numeric sequence of length >=2, indicating what PC components to plot in the loadingplot.
#########################
...
sim.vars <- "C_Water"					## which variables should be use to group the data? Leave empty c() or set to NULL for using all available class-variables, or provide a character vector with valid class variable names to group along these variables; e.g. "C_Group", or c("C_Group", "C_FooBar", ...)
...
pls.ncomp <- 2							## number of components, leave at NULL for automatic detection of optimal number of components
pls.regOn <- c("Y_waterTemp")			## which variables should be used to regress on? Leave empty c() or set to NULL for using all numerical variables, or provide a character vector with valid numerical variable names to regress on these variables; e.g. "Y_Temp", or c("Y_Temp", "Y_FooBar", ...)
...
# plotting
pls.colorBy <- "C_waterTemp"			## What class-variable should be used for coloring in the RMSEP and RMSECV plots? Leave empty c() or set to NULL for no coloring, or provide a character length one with a valid class variable name to color along this variable; e.g. "C_Group"
...
aqg.vars <- "C_waterTemp"				## which variables should be use to group the data? Provide a character vector with valid class variable names to group along these variables; e.g. "C_Group", or c("C_Group", "C_FooBar", ...)
